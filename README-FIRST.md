...
git diff --check
git add .
git commit -m "Add observability, rollback and native store readiness"
...
8. Record each deployed Worker version and set `ROLLBACK_APP_VERSION`; exercise the protected rollback workflow in staging before production.
