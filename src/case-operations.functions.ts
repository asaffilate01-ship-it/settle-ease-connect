...
export const requestInsuranceQuote = createServerFn({ method: "POST" })
  .validator((raw) => insuranceQuoteSchema.parse(raw))
  .handler(async ({ data }) => {
    await enforceRateLimit({ scope: "partner-insurance-quote", limit: 10, windowSeconds: 3600 });
    return quoteFromFeather(data);
  });

export const requestTaxHandoff = createServerFn({ method: "POST" })
  .validator((raw) => taxHandoffSchema.parse(raw))
  .handler(async ({ data }) => {
    await enforceRateLimit({ scope: "partner-tax-handoff", limit: 10, windowSeconds: 3600 });
    return handoffToTaxfix(data);
  });

export const requestCareBooking = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((raw) => careBookingSchema.parse(raw))
  .handler(async ({ data, context }) => {
    await enforceRateLimit({
      scope: "partner-care-booking",
      limit: 10,
      windowSeconds: 3600,
