...
async function assertFinance(context: { supabase: any; userId: string }) {
  ...
  if (!admin && !finance) throw new Error("Forbidden: finance role required");
}

function assertSandboxPayoutLedger(): void {
  if (
    getServerStripeEnvironment() !== "sandbox"
    || process.env.ENABLE_SANDBOX_PAYOUT_LEDGER !== "true"
  ) {
    throw new Error(
      "Live fund release is disabled until a regulated payment/custody flow and connected payout accounts are configured.",
    );
  }
}
...
export const releaseEscrow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertFinance(context);
    assertSandboxPayoutLedger();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: releasedAt, error } = await supabaseAdmin.rpc(
      "queue_invoice_payout" as never,
      {
...
/**
 * Queue all eligible sandbox-ledger invoices whose `paid_at` is older than N
 * days. Live payment movement is deliberately blocked above.
 */
    ...
    for (const inv of eligible ?? []) {
      const { error: releaseError } = await supabaseAdmin.rpc(
        "queue_invoice_payout" as never,
        {
          _invoice_id: inv.id,
          _actor_user_id: context.userId,
          _notes: `Sandbox ledger release after ${data.olderThanDays}d`,
        } as never,
      );
      if (releaseError) {
...
export const createCheckoutSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<CheckoutSessionResult> => {
    try {
      ...
      const stripe = createStripeClient(getServerStripeEnvironment());
      const returnUrl = safePaymentReturnUrl(
        data.returnUrl,
...
export const createPortalSession = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: { returnUrl?: string } | undefined) => data ?? {})
  .handler(async ({ data, context }): Promise<PortalSessionResult> => {
    ...
    try {
      const stripe = createStripeClient(getServerStripeEnvironment());
      const returnUrl = safePaymentReturnUrl(data.returnUrl, "/app/account");
