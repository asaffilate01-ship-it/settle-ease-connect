...
export const askFamilyAssistant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: services } = await context.supabase
      ...
      .eq("status", "active")
      .limit(50);
    const contextText = (services ?? [])
      .map(
        (service) =>
          `• ${service.name} (${service.slug}) — ${service.short_description ?? ""}; timeline: ${service.typical_timeline ?? "unknown"}; fees: ${service.official_fees ?? "check official source"}`,
      )
      .join("\n");
    ...
    const answer = await callGateway([
      {
        ...
        content:
          ...
          "Answer in the language used by the member. Give short, practical steps, identify documents and deadlines, and name the responsible official authority. " +
          "Never claim to be a lawyer, tax adviser, doctor or insurance broker. Do not invent eligibility, fees, offices or deadlines. " +
          "When a fact is uncertain or current, tell the member to confirm it with the official authority or their case manager. " +
          `Available internal service summaries:\n${contextText}`,
      },
      ...data.history,
