...
export const listMyCommissions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("agent_commissions")
      .select(
        "id, period_month, product, gross_eur, commission_rate, commission_eur, status, paid_at, notes",
      )
      .eq("agent_user_id", userId)
...
export const addManualReferral = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: active, error: activeErr } = await supabase.rpc("is_active_agent", {
      _user_id: userId,
    });
    if (activeErr) throw new Error(activeErr.message);
    if (!active)
      throw new Error("Your agent account is not active. Contact BeistandPlus to reactivate.");
...
export const getMyAgentFunnel = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const stages = [
      ...
      { key: "subscribed", label: "Subscribed", count: subscribed },
      {
        ...
        count: subs.filter(
          (s) =>
            s.status === "active" &&
            Date.now() - new Date(s.created_at).getTime() > 30 * 86_400_000,
        ).length,
      },
    ];
    ...
    referrals.forEach((r) => {
      byProduct[r.product ?? "other"] = (byProduct[r.product ?? "other"] ?? 0) + 1;
    });

    // Group by source
    const bySource: Record<string, number> = {};
    referrals.forEach((r) => {
      bySource[r.source ?? "other"] = (bySource[r.source ?? "other"] ?? 0) + 1;
    });
    ...
    return {
      ...
      churned,
      earnings: {
        paidEur: Math.round(paidComm * 100) / 100,
        pendingEur: Math.round(pendingComm * 100) / 100,
      },
