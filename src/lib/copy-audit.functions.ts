...
export const listCopyAudit = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data } = await context.supabase
      ...
      .select(
        "id, surface, route_path, domain, status, findings, reviewer_user_id, reviewed_at, notes, updated_at",
      )
      .order("domain")
      .order("surface");
    return data ?? [];
  ...
  .validator((d) =>
    z
      .object({
        ...
        findings: z
          .array(
            z.object({ severity: z.enum(["info", "warn", "block"]), text: z.string().max(600) }),
          )
          .optional(),
      })
      .parse(d),
