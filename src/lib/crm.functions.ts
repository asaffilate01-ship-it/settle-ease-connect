...
export const crmInbox = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [newLeads, unassigned, followUpsDue, slaBreached, openComplaints, stageCounts] = await Promise.all([
      context.supabase.from("crm_leads").select("id,reference,lead_type,stage,priority,language,created_at,owner_user_id,notes")
        .order("created_at", { ascending: false }).limit(20),
      context.supabase.from("crm_leads").select("id,reference,lead_type,stage,created_at").is("owner_user_id", null).limit(20),
      context.supabase.from("crm_follow_ups").select("id,title,due_at,channel,assignee_user_id,lead_id,contact_id,user_id")
        .eq("done", false).lte("due_at", now).order("due_at").limit(30),
      context.supabase.from("crm_leads").select("id,reference,lead_type,stage,sla_due_at").lte("sla_due_at", now)
        .not("stage", "in", "(onboarded,ongoing,lost)").limit(20),
      context.supabase.from("crm_complaints").select("id,reference,subject,severity,status,opened_at").eq("status", "open").limit(20),
      context.supabase.from("crm_leads").select("stage"),
    ]);
    const counts: Record<string, number> = {};
    (stageCounts.data ?? []).forEach((r: { stage: string }) => { counts[r.stage] = (counts[r.stage] ?? 0) + 1; });
    return {
...
export const listCrmContacts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    let query = context.supabase.from("crm_contacts")
      .select("id, full_name, email, phone, preferred_language, city, source, campaign, merged_into_user_id, created_at, tags")
    ...
    let profileQuery = context.supabase.from("profiles").select("id, full_name, preferred_language, city, avatar_url, created_at").limit(50);
    if (data.q && data.q.length > 0) {
...
export const getCrmContact = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [contact, leads, activities, consents, followUps, complaints] = await Promise.all([
      ...
      context.supabase.from("crm_leads").select("*").eq("contact_id", data.id).order("created_at", { ascending: false }),
      context.supabase.from("crm_activities").select("*").eq("contact_id", data.id).order("occurred_at", { ascending: false }).limit(100),
      context.supabase.from("crm_consents").select("*").eq("contact_id", data.id).order("granted_at", { ascending: false }),
      context.supabase.from("crm_follow_ups").select("*").eq("contact_id", data.id).order("due_at"),
      context.supabase.from("crm_complaints").select("*").eq("contact_id", data.id).order("opened_at", { ascending: false }),
    ]);
...
export const createCrmContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { data: row, error } = await context.supabase.from("crm_contacts")
...
export const getCrmUserRecord = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [profile, roles, family, cases_, subs, insLeads, funLeads, funPols, complaints, consents, activities, followUps, invoices, notifPrefs] = await Promise.all([
      ...
      context.supabase.from("cases").select("id, reference, title, case_type, status, urgent, opened_at, updated_at, language").or(`client_user_id.eq.${uid},case_manager_user_id.eq.${uid}`).order("updated_at", { ascending: false }),
      context.supabase.from("subscriptions").select("*").eq("user_id", uid).order("current_period_end", { ascending: false }),
      ...
      context.supabase.from("crm_complaints").select("*").eq("user_id", uid).order("opened_at", { ascending: false }),
      context.supabase.from("crm_consents").select("*").eq("user_id", uid).order("granted_at", { ascending: false }),
      context.supabase.from("crm_activities").select("*").eq("user_id", uid).order("occurred_at", { ascending: false }).limit(100),
      ...
      context.supabase.from("notification_preferences").select("*").eq("user_id", uid).maybeSingle(),
    ]);
...
export const listCrmLeads = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    let q = context.supabase.from("crm_leads").select("*").order("updated_at", { ascending: false }).limit(500);
...
export const getCrmLead = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [lead, activities, followUps, consents] = await Promise.all([
      ...
      context.supabase.from("crm_activities").select("*").eq("lead_id", data.id).order("occurred_at", { ascending: false }),
    ...
    if (lead.data.contact_id) {
      const { data: c } = await context.supabase.from("crm_contacts").select("*").eq("id", lead.data.contact_id).maybeSingle();
      contact = c;
    }
    let profile = null;
    if (lead.data.user_id) {
      const { data: p } = await context.supabase.from("profiles").select("*").eq("id", lead.data.user_id).maybeSingle();
      profile = p;
    }
    ...
    return {
      ...
      activities: activities.data ?? [],
      followUps: followUps.data ?? [],
      consents: (consents.data ?? []).filter((c: { contact_id: string | null; user_id: string | null }) =>
...
export const createCrmLead = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) =>
    ...
    }).refine((v) => v.contact_id || v.user_id, { message: "Lead must reference a contact or user" }).parse(d),
  )
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { data: row, error } = await context.supabase.from("crm_leads")
      .insert({ ...data, created_by: context.userId, owner_user_id: context.userId }).select().single();
    if (error) throw new Error(error.message);
    return row;
  });
...
export const advanceCrmLeadStage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.stage === "insurance_referral_offered" || data.stage === "referred_to_partner") {
      const { data: lead } = await context.supabase.from("crm_leads").select("contact_id,user_id").eq("id", data.id).maybeSingle();
      if (lead) {
        const base = context.supabase.from("crm_consents").select("id").eq("purpose", "insurance_referral").is("revoked_at", null);
    ...
    await context.supabase.from("crm_activities").insert({
      lead_id: data.id, kind: "system", subject: `Stage → ${data.stage}`,
...
export const assignCrmLead = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { error } = await context.supabase.from("crm_leads").update({ owner_user_id: data.owner_user_id }).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const addCrmActivity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { data: row, error } = await context.supabase.from("crm_activities")
      .insert({ ...data, actor_user_id: context.userId }).select().single();
    if (error) throw new Error(error.message);
    return row;
  });
...
export const scheduleFollowUp = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await context.supabase.from("crm_follow_ups")
      .insert({ ...data, assignee_user_id: assignee, created_by: context.userId }).select().single();
    if (error) throw new Error(error.message);
    return row;
  });
...
export const completeFollowUp = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid(), done: z.boolean().default(true) }).parse(d))
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { error } = await context.supabase.from("crm_follow_ups")
      .update({ done: data.done, done_at: data.done ? new Date().toISOString() : null }).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const recordConsent = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) =>
    z.object({
      ...
      language: z.string().max(5).optional().nullable(),
    }).refine((v) => v.contact_id || v.user_id, { message: "Consent requires contact_id or user_id" }).parse(d),
  )
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { data: row, error } = await context.supabase.from("crm_consents")
      .insert({ ...data, actor_user_id: context.userId }).select().single();
    if (error) throw new Error(error.message);
    return row;
  });
...
export const revokeConsent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { error } = await context.supabase.from("crm_consents")
...
export const listCrmComplaints = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { data, error } = await context.supabase.from("crm_complaints")
      .select("*").order("opened_at", { ascending: false }).limit(500);
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const updateComplaintStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const patch: { status: typeof data.status; resolution?: string | null; satisfaction_score?: number | null; closed_at?: string } = { status: data.status };
    ...
    if (["resolved","rejected","withdrawn"].includes(data.status)) patch.closed_at = new Date().toISOString();
...
export const createComplaint = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensureInternal(context.supabase, context.userId);
    const { data: row, error } = await context.supabase.from("crm_complaints")
      .insert({ ...data, created_by: context.userId }).select().single();
    if (error) throw new Error(error.message);
    return row;
  });
