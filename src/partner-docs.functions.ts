...
const InputSchema = z.object({
  ...
  texts: z.array(z.string().min(1).max(1200)).min(1).max(30),
}).refine((value) => value.texts.reduce((total, text) => total + text.length, 0) <= 12_000, {
  message: "Translation batch is too large",
});
...
export const translateBatch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await enforceRateLimit({ scope: "translation", limit: 20, windowSeconds: 600 });
