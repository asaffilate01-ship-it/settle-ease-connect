...
export const Route = createFileRoute("/_authenticated/portal/escrow")({
  head: () => ({ meta: [{ title: "Payout ledger — Staff" }] }),
...
function EscrowPage() {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => {
      toast.success("Sandbox payout ledger entry queued");
  ...
  const autoMut = useMutation({
    ...
    onSuccess: (r: any) => {
      toast.success(`Queued ${r.released_count} sandbox ledger item(s)${r.failed_count ? ` · ${r.failed_count} failed` : ""}`);
      qc.invalidateQueries({ queryKey: ["portal", "escrow"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-display text-2xl font-semibold">Payout release ledger</h1>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            Sandbox-only accounting workflow for approved expert payouts. This does not move client funds and is blocked in live mode until a regulated payment flow is connected.
          </p>
        </div>
        {tab === "held_escrow" && (
          <Button
            ...
            title="Queue sandbox ledger entries for eligible invoices older than 14 days"
          >
            <Clock className="mr-1.5 h-4 w-4" />
            {autoMut.isPending ? "Queuing…" : "Queue eligible >14d"}
          </Button>
        )}
      </div>
      ...
      <div className="flex gap-2 border-b border-border/60">
        {(["held_escrow", "released"] as Tab[]).map((k) => (
          ...
          >
            {k === "held_escrow" ? "Awaiting approval" : "Ledger queued"}
          </button>
        ))}
      </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  {tab === "held_escrow" ? (
                    ...
                    >
                      <CheckCircle2 className="mr-1.5 h-4 w-4" /> Queue payout
                    </Button>
                  ) : (
