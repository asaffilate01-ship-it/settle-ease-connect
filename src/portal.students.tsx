...
// Production bookings require a configured provider API. Local confirmations
// require an explicit development-only flag.

import { partnerDemosEnabled, type CareTranslationBooking, type CareTranslationConfirmation, type PartnerStatus } from "./types";
...
export function careTranslationStatus(): PartnerStatus {
  ...
  return {
    ...
    configured,
    gap: configured ? undefined : demo ? "Local demo bookings are enabled." : "No contracted interpreting API is configured.",
  };
}
...
export async function bookCareTranslation(input: CareTranslationBooking): Promise<CareTranslationConfirmation> {
  ...
  if (apiKey) {
    ...
    throw new Error(`Interpreting partner unavailable (${resp.status}).`);
  }

  if (!partnerDemosEnabled()) {
    throw new Error("Interpreting booking is not available.");
  }

  // Development-only confirmation; never enabled in production.
  return {
