  ...
  const products: { value: ProductValue; label: string }[] = [
    {
      ...
      label: t("agent.products.basic", { defaultValue: "Subscription — Basic €5/mo" }),
    },
    {
      value: "subscription_plus",
      label: t("agent.products.plus", { defaultValue: "Subscription — Plus €10/mo" }),
    },
    {
      value: "subscription_complete",
      label: t("agent.products.complete", { defaultValue: "Subscription — Complete €25/mo" }),
    },
    {
      value: "funeral_cover",
      label: t("agent.products.funeral", { defaultValue: "Funeral cover" }),
    },
  ...
  const add = useMutation({
    ...
    onError: (e) =>
      toast.error(
        e instanceof Error
          ? e.message
          : t("agent.clients.saveFail", { defaultValue: "Could not save" }),
      ),
  });
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <header>
        <h1 className="font-display text-3xl font-semibold">
          {t("agent.clients.title", { defaultValue: "My clients" })}
        </h1>
        <p className="mt-1 text-muted-foreground">
          {t("agent.clients.subtitle", {
            defaultValue:
              "Every subscription you sell earns {{rate}}% of the monthly fee, for as long as the client stays with us. Note: we sell funeral cover, not funeral insurance.",
            rate,
          })}
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-border/60 bg-card p-5">
        ...
        >
          <div>
            <Label htmlFor="agent-client-email">
              {t("agent.clients.emailLabel", { defaultValue: "Client email" })}
            </Label>
            <Input
              id="agent-client-email"
              type="email"
              value={email}
          ...
          <div>
            <Label htmlFor="agent-client-product">
              {t("agent.clients.productLabel", { defaultValue: "Product" })}
            </Label>
            <select
          ...
          <div className="sm:col-span-2">
            <Label htmlFor="agent-client-notes">
              {t("agent.clients.notesLabel", { defaultValue: "Notes (optional)" })}
            </Label>
            <Input
              id="agent-client-notes"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder={t("agent.clients.notesPh", {
                defaultValue: "Language, city, follow-up date…",
              })}
            />
          </div>
          <div className="sm:col-span-2">
            <Button type="submit" disabled={add.isPending}>
              {add.isPending
                ? t("agent.clients.saving", { defaultValue: "Saving…" })
                : t("agent.clients.submit", { defaultValue: "Log referral" })}
            </Button>
          </div>
        </form>
      </section>
              ...
              {data.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-6 text-center text-muted-foreground">
                    {t("agent.clients.empty", { defaultValue: "No referrals yet." })}
                  </td>
                </tr>
              )}
            </tbody>
