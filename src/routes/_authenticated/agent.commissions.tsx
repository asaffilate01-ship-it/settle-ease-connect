      ...
      <header>
        ...
        <p className="mt-1 text-muted-foreground">
          {t("agent.commissions.subtitle", {
            defaultValue: "Paid monthly, calculated as {{rate}}% of the monthly subscription fee for each client you referred. Third-party costs (insurance, funeral cover) are excluded from the commission base.",
            rate,
          })}
        </p>
      </header>
        ...
        <table className="w-full min-w-[720px] text-sm">
          <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
            <tr>
              ...
              <th className="p-3 text-right">{t("agent.commissions.gross", { defaultValue: "Gross" })}</th>
              <th className="p-3 text-right">{t("agent.commissions.rate", { defaultValue: "Rate" })}</th>
              <th className="p-3 text-right">{t("agent.commissions.commission", { defaultValue: "Commission" })}</th>
          ...
          <tbody>
            ...
            {data.length === 0 && (
              <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">
                {t("agent.commissions.empty", { defaultValue: "No commissions yet — they appear after your first referred client’s subscription is billed." })}
              </td></tr>
            )}
          </tbody>
        </table>
