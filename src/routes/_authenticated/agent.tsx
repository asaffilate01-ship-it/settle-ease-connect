...
function AgentLayout() {
  ...
  const name =
    profile?.full_name ||
    user?.email?.split("@")[0] ||
    t("agent.defaultName", { defaultValue: "Agent" });
  ...
  const NAV: { to: string; label: string; icon: typeof Home; exact?: boolean }[] = [
    {
      to: "/agent",
      label: t("agent.nav.overview", { defaultValue: "Overview" }),
      icon: Home,
      exact: true,
    },
    {
      to: "/agent/leads",
      label: t("agent.nav.leads", { defaultValue: "Leads & funnel" }),
      icon: Filter,
    },
    {
      to: "/agent/clients",
      label: t("agent.nav.clients", { defaultValue: "My clients" }),
      icon: Users,
    },
    {
      to: "/agent/commissions",
      label: t("agent.nav.commissions", { defaultValue: "Commissions" }),
      icon: TrendingUp,
    },
    {
      to: "/agent/link",
      label: t("agent.nav.link", { defaultValue: "Referral link" }),
      icon: LinkIcon,
    },
  ];
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border/60 bg-background/80 px-4 backdrop-blur sm:px-6">
          ...
          <Link to="/agent" className="flex items-center gap-2 lg:hidden">
            <span className="font-display text-lg font-semibold">
              {t("agent.shortLabel", { defaultValue: "Agent" })}
            </span>
          </Link>
