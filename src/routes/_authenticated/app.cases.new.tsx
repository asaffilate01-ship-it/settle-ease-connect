...
function NewCase() {
  ...
  const [form, setForm] = useState({
    title: "",
    case_type: "bereavement" as (typeof CASE_TYPES)[number][0],
    summary: "",
    urgent: false,
    language: "en",
    city: "",
    ...
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          A case manager responds during the published service hours.
        </p>
      </div>
      <form
      ...
      >
        <Field label="Short title">
          <Input
            ...
            placeholder="e.g. Repatriation to Istanbul"
          />
        </Field>
        <Field label="Type">
          <Select
            value={form.case_type}
            onValueChange={(v) => setForm({ ...form, case_type: v as typeof form.case_type })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <Field label="Preferred language">
          <Select value={form.language} onValueChange={(v) => setForm({ ...form, language: v })}>
            ...
            <SelectContent>
              {["de", "en", "tr", "ur", "hi", "pa", "ps", "ar", "ku", "ru", "uk"].map((l) => (
        ...
        <label className="flex items-center gap-2 text-sm">
          ...
          />
          <span>This is urgent (prioritise for triage)</span>
        </label>
        <div className="flex justify-end gap-2 pt-2">
          <Button type="button" variant="outline" onClick={() => navigate({ to: "/app/cases" })}>
            Cancel
          </Button>
