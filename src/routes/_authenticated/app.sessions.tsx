    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-soft">
        <div className="flex items-center gap-2 font-medium">
          <Key className="h-4 w-4" /> {t("pages.sessions.passkeys")}
        </div>
        <div className="mt-3 space-y-2">
          {allFactors.length === 0 && (
            <p className="text-sm text-muted-foreground">{t("pages.sessions.noneEnrolled")}</p>
          )}
      ...
      <div>
        <div className="mb-3 flex items-center gap-2 font-medium">
          <Shield className="h-4 w-4" /> {t("pages.sessions.recent")}
        </div>
        <div className="space-y-2">
          {sessions.length === 0 && (
            <p className="text-sm text-muted-foreground">{t("pages.sessions.noEvents")}</p>
          )}
          {sessions.map((s) => (
            <div
              key={s.id}
            ...
            >
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {s.device_label ?? s.user_agent ?? t("pages.sessions.unknownDevice")}
                </div>
              </div>
              <div className="text-xs text-muted-foreground">
