    ...
    return (
      <div className="p-6 max-w-lg mx-auto space-y-4">
        <Alert>
          ...
          <AlertDescription>
            Your account is not linked to a partner organisation yet. If you were invited by email,
            accept the invitation below — otherwise ask your BeistandPlus contact to send you one.
          </AlertDescription>
        </Alert>
              ...
              {!a.accepted_at && !a.declined_at && (
                <>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      mut.mutate({ id: a.id, response: "decline", reason: "Not available" })
                    }
                  >
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="e.g. Berufshaftpflicht 2026"
                className="mt-1"
              />
            </div>
            <div>
        ...
        <CardContent className="space-y-2">
          {data.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No documents yet. Upload licence, insurance certificate, bank details, and
              registration.
            </p>
          ) : (
            data.map((d: any) => (
...
function PartnerServices({ orgId, isAdmin }: { orgId: string; isAdmin: boolean }) {
  const qc = useQueryClient();
  const list = useQuery({
    queryKey: ["partner", "cats", orgId],
    queryFn: () => listPartnerCategories({ data: { orgId } }),
  });
    ...
    mutationFn: () =>
      add({
        data: {
          orgId,
          category,
          translatorServiceType: category === "translator" ? tType : undefined,
        },
      }),
    onSuccess: () => {
      toast.success("Added");
      qc.invalidateQueries({ queryKey: ["partner", "cats", orgId] });
    },
    onError: (e: any) => toast.error(e?.message ?? "Failed"),
  });
  ...
  const updMut = useMutation({
    mutationFn: (v: {
      ...
      active?: boolean;
      swornCourts?: string[];
      translatorServiceType?: (typeof TRANSLATOR_TYPES)[number] | null;
    }) => update({ data: v }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["partner", "cats", orgId] });
    },
          ...
          {list.isLoading ? (
            ...
            (list.data ?? []).map((r: any) => (
              <div key={r.id} className="rounded-md border p-3 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className="font-medium text-sm">
                      ...
                      {r.translator_service_type && (
                        <span className="text-muted-foreground">
                          {" "}
                          · {r.translator_service_type.replaceAll("_", " ")}
                        </span>
                      )}
                    </div>
                    {r.category === "translator" && r.translator_service_type === "sworn" && (
                      <div className="text-xs text-muted-foreground">
                        Sworn courts:{" "}
                        {r.sworn_courts?.length ? r.sworn_courts.join(", ") : "none listed"}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1">
                      <Switch
                        checked={r.active}
                        onCheckedChange={(v) => isAdmin && updMut.mutate({ id: r.id, active: v })}
                        disabled={!isAdmin}
                      />
                ...
                {isAdmin &&
                  r.category === "translator" &&
                  r.translator_service_type === "sworn" && (
                      ...
                      {(r.sworn_courts ?? []).map((c: string) => (
                        <Badge
                          ...
                          onClick={() =>
                            updMut.mutate({
                              id: r.id,
                              swornCourts: (r.sworn_courts ?? []).filter((x: string) => x !== c),
                            })
                          }
                        >
                          {c} ✕
...
function PartnerCoverage({ orgId, isAdmin }: { orgId: string; isAdmin: boolean }) {
  const qc = useQueryClient();
  const list = useQuery({
    queryKey: ["partner", "regions", orgId],
    queryFn: () => listPartnerRegions({ data: { orgId } }),
  });
    ...
    mutationFn: () =>
      add({
        data: {
          ...
          postalPrefix: postalPrefix || undefined,
          radiusKm: radius ? Number(radius) : undefined,
        },
      }),
    onSuccess: () => {
      toast.success("Region added");
      qc.invalidateQueries({ queryKey: ["partner", "regions", orgId] });
      setCity("");
      setBundesland("");
      setPostalPrefix("");
      setRadius("");
    ...
    <div className="space-y-4">
      {isAdmin && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <MapPin className="h-4 w-4" /> Add coverage area
            </CardTitle>
          </CardHeader>
          <CardContent className="grid gap-3 md:grid-cols-4">
            ...
            <div>
              <Label className="text-xs">PLZ prefix</Label>
              <Input
                value={postalPrefix}
            ...
            (list.data ?? []).map((r: any) => (
              <div key={r.id} className="flex items-center justify-between rounded-md border p-3">
                <div className="text-sm">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {r.postal_prefix ? `PLZ ${r.postal_prefix}*` : ""}{" "}
                    {r.radius_km ? ` · ${r.radius_km} km radius` : ""}
                  </div>
                </div>
                {isAdmin && (
                  <Button
...
function PartnerAvailability({ orgId, isAdmin }: { orgId: string; isAdmin: boolean }) {
  const qc = useQueryClient();
  const list = useQuery({
    queryKey: ["partner", "avail", orgId],
    queryFn: () => listPartnerAvailability({ data: { orgId } }),
  });
    ...
    mutationFn: () =>
      add({
        data: {
          ...
          startTime: start,
          endTime: end,
          acceptsUrgent: urgent,
        },
      }),
    onSuccess: () => {
      toast.success("Slot added");
      qc.invalidateQueries({ queryKey: ["partner", "avail", orgId] });
    },
    onError: (e: any) => toast.error(e?.message ?? "Failed"),
  });
  const rmMut = useMutation({
