...
function AuditPage() {
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      listFn({ data: { q: q || undefined, entity_type: entityType || undefined, limit: 300 } }),
  });
      ...
      <div className="mx-auto max-w-6xl space-y-5">
        <header>
          ...
          <p className="text-sm text-muted-foreground">
            Every sensitive action across the platform, exportable for GDPR/audit.
          </p>
        </header>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={exportCsv}
            className="h-9 rounded-md border px-3 text-sm hover:bg-accent/40"
          >
            Export CSV
          </button>
        </div>
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-t">
                    ...
                    <td className="px-3 py-2">
                      {r.actor_email ?? r.actor_user_id?.slice(0, 8) ?? "system"}
                    </td>
