...
function ComplaintsPage() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Complaints ({data.length})</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        {data.length === 0 && (
          <p className="text-sm text-muted-foreground">No complaints on file. Nice.</p>
        )}
        {data.map((c) => (
          <div
            key={c.id}
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground">
                {c.reference} · severity {c.severity} · opened{" "}
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                value={c.status}
                onValueChange={(v) => mut.mutate({ id: c.id, status: v as any })}
              >
