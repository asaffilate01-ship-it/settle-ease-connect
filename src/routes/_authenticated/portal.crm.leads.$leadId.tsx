...
function LeadDetail() {
  ...
  const followMut = useMutation({
    mutationFn: () =>
      followUpFn({
        data: { title: followTitle, due_at: new Date(followWhen).toISOString(), lead_id: leadId },
      }),
    onSuccess: () => {
      setFollowTitle("");
      setFollowWhen("");
    ...
    <div className="space-y-6">
        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-3 p-6">
          <div>
            ...
            <div className="mt-1 text-sm text-muted-foreground">
              {l.lead_type} · {l.language ?? "—"} · priority {l.priority}
            </div>
          </div>
        ...
        <div className="flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/5 p-3 text-sm">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              This lead has crossed into regulated insurance territory. Only certified insurance
              advisors may action further changes. Introductory support staff should not give advice
      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <CardContent className="space-y-3">
            ...
            >
              ...
              <div className="flex justify-end">
                <Button size="sm" type="submit" disabled={noteMut.isPending || !note.trim()}>
                  Log note
                </Button>
              </div>
            </form>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                <AlertTriangle className="mr-1 inline h-3 w-3" /> Insurance-referral stages require
          ...
          <Card>
            ...
            <CardContent>
              <form
                ...
                onSubmit={(e) => {
                  e.preventDefault();
                  if (followTitle && followWhen) followMut.mutate();
                }}
              >
                <Input
                ...
                <Button
                  size="sm"
                  type="submit"
                  disabled={followMut.isPending || !followTitle || !followWhen}
                  className="w-full"
                >
                  Schedule
                </Button>
              ...
              <div className="mt-3 space-y-1">
                {data.followUps.map((f: any) => (
                  <div key={f.id} className="text-xs flex justify-between">
                    <span className={f.done ? "line-through text-muted-foreground" : ""}>
                      {f.title}
                    </span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Consents on file</CardTitle>
            </CardHeader>
            <CardContent className="space-y-1 text-sm">
              {data.consents.length === 0 && (
                <p className="text-muted-foreground text-xs">
                  None. Advance to "consented" only after recording consent on the contact page.
                </p>
              )}
