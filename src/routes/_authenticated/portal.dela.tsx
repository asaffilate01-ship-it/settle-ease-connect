      ...
      <div className="space-y-6">
        <header>
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Regulated funeral-expense insurance referral to DELA / authorised intermediary.
            BeistandPlus is an introducer only — we do not advise, recommend or interpret policy
            wording.
          </p>
        </header>
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="font-medium text-amber-800 dark:text-amber-200">
            Introducer only — no advice
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            Unlicensed staff must not recommend a policy, compare products as advice, state
            suitability, interpret exclusions, change insurer wording, or complete regulated
            declarations for the customer. Post-consent stages are database-enforced to licensed
            advisors only.
          </p>
        </div>
      </div>
...
function NewReferralCard({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const start = useMutation({
    mutationFn: () =>
      startFn({ data: { fullName, email: email || undefined, phone: phone || undefined } }),
    onSuccess: (row) => {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm">Start a new DELA referral</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        <p className="text-[11px] text-muted-foreground">
          Clicking <em>Show disclosure &amp; start</em> confirms the customer has been shown the
          referral disclosure (v{DELA_DISCLOSURE_VERSION}) and the privacy notice (v
          {DELA_PRIVACY_NOTICE_VERSION}).
        </p>
        <Input
        ...
        <Button
          className="w-full"
          disabled={!fullName || start.isPending}
...
function ReferralDetail({ id }: { id: string }) {
  ...
  const [evidence, setEvidence] = useState(
    "Customer confirmed marketing consent verbally on call, recorded in notes.",
  );
  ...
  const infoMut = useMutation({
    mutationFn: (payload: { withContact: boolean }) =>
      infoFn({
        data: {
          referralId: id,
          ...
          contactMethod: payload.withContact
            ? (contactMethod as "email" | "phone" | "whatsapp" | "post")
            : undefined,
  ...
  const sendMut = useMutation({
    ...
    onSuccess: (row) => {
      toast.success(`Referral ${row.reference} sent to partner`);
      invalidate();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <StageCard
        step="1"
        title="Referral disclosure displayed"
        done={Boolean(r.disclosure_shown_at)}
      >
        <p>
          Version <b>{r.disclosure_version ?? "—"}</b> at{" "}
          {r.disclosure_shown_at ? new Date(r.disclosure_shown_at).toLocaleString() : "—"}. Privacy
          notice <b>{r.privacy_notice_version ?? "—"}</b>.
        </p>
      </StageCard>
      ...
      <StageCard step="2" title="Marketing consent obtained" done={hasConsent}>
        {hasConsent ? (
          <p>
            Consent recorded {new Date(r.marketing_consent_at!).toLocaleString()}:{" "}
            <em>{r.marketing_consent_evidence}</em>
          </p>
        ) : (
          <div className="space-y-2">
            <Textarea
              ...
              placeholder="Consent evidence (how/where/when)"
            />
            <Button
              size="sm"
              onClick={() => consentMut.mutate()}
              disabled={consentMut.isPending || !evidence}
            >
              Record consent
            </Button>
          </div>
      ...
      <StageCard
        step="3"
        title="Basic non-advisory information"
        done={Boolean(r.age || r.postcode || r.household_kind)}
      >
        <p className="text-[11px] text-muted-foreground mb-2">
          <Lock className="h-3 w-3 inline mr-1" /> Only name, contactable details, age, household
          composition and postcode. Do not collect health / suitability data — DELA collects that
          directly.
        </p>
        <div className="grid gap-2 sm:grid-cols-3">
          ...
          />
          <Input
            placeholder="Household (single/family)"
            value={household}
        ...
        <Button
          size="sm"
          className="mt-2"
          onClick={() => infoMut.mutate({ withContact: false })}
      ...
      <StageCard step="4" title="Customer selects contact method" done={hasContact}>
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <Input
            placeholder="Preferred time (e.g. weekdays after 18:00)"
            value={contactTime}
        ...
        <Button
          size="sm"
          className="mt-2"
          onClick={() => infoMut.mutate({ withContact: true })}
          disabled={!contactMethod || infoMut.isPending}
        >
          Confirm contact method
        </Button>
        {hasContact ? (
          <p className="text-xs text-muted-foreground mt-2">
            Preferred: {r.contact_method} · {r.contact_time_preference ?? "no time set"}
          </p>
        ) : null}
      </StageCard>
      ...
      <StageCard step="5" title="Send referral securely to DELA" done={isSent}>
        {isSent ? (
          <p>
            Sent {new Date(r.sent_to_partner_at!).toLocaleString()} · reference{" "}
            <b className="font-mono">{r.reference}</b>
          </p>
        ) : (
          <div>
            <p className="text-[11px] text-muted-foreground mb-2">
              Requires marketing consent and contact method.
            </p>
            <Button
              size="sm"
              onClick={() => sendMut.mutate()}
              disabled={!hasConsent || !hasContact || sendMut.isPending}
            >
      ...
      <Card className="border-primary/30">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            <Lock className="h-4 w-4 text-primary" /> Licensed advisor only — policy outcome &
            commission
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-[11px] text-muted-foreground">
            Database-enforced. Only users with role <code>insurance_admin</code> or{" "}
            <code>admin</code> can record the fields below.
          </p>
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              size="sm"
              variant="outline"
              disabled={!isSent || outcomeMut.isPending}
              ...
              <Button
                ...
                onClick={() =>
                  outcomeMut.mutate({
                    ...
                    commissionAmountEur: commission ? Number(commission) : undefined,
                  })
                }
              >
                Commission due
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          <span
            className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-[11px] ${done ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}
          >
