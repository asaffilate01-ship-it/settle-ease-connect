...
function EscrowPage() {
  ...
  const autoMut = useMutation({
    ...
    onSuccess: (r: any) => {
      toast.success(
        `Auto-released ${r.released_count} invoice(s)${r.failed_count ? ` · ${r.failed_count} failed` : ""}`,
      );
      qc.invalidateQueries({ queryKey: ["portal", "escrow"] });
    },
    onError: (e: Error) => toast.error(e.message),
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Client funds held on paid invoices. Release once expert work is delivered — creates a
              pending payout row.
            </p>
          </div>
          {tab === "held_escrow" && (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 text-xs text-muted-foreground">
                      {inv.experts?.profession} · Case {inv.case_id.slice(0, 8)} · Invoice{" "}
                      {inv.id.slice(0, 8)}
                    </div>
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs">
                      ...
                      <span>
                        ...
                        <span className="font-medium">
                          €{Number(inv.platform_fee_eur ?? 0).toFixed(2)}
                        </span>
                      </span>
                      <span>
                  ...
                  <div className="flex items-center gap-2">
                    {tab === "held_escrow" ? (
                      <Button size="sm" onClick={() => mut.mutate(inv.id)} disabled={mut.isPending}>
