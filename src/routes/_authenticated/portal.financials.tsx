        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Revenue, expenses and P&amp;L across BeistandPlus — subscriptions, case invoices,
              expert payouts and agent commissions.
            </p>
          </div>
            ...
            {outstanding && (
              <div className="rounded-xl border border-border/60 bg-card p-4">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <MiniStat
                    label="Pending expert payouts"
                    value={fmt(outstanding.pendingExpertPayouts)}
                  />
                  <MiniStat
                    label="Pending agent commissions"
                    value={fmt(outstanding.pendingAgentCommissions)}
                  />
                </div>
              </div>
            )}
                ...
                <table className="w-full text-sm">
                    ...
                    {rows.map((r) => (
                      <tr key={r.key} className="border-t border-border/40">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {fmt(r.subscriptionRevenue)}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {fmt(r.invoiceRevenue)}
                        </td>
                        ...
                        <td className="px-3 py-2 text-right tabular-nums text-muted-foreground">
                          −{fmt(r.agentCommissions)}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums text-emerald-700">
                          {fmt(r.platformFees)}
                        </td>
                        <td
                          className={`px-3 py-2 text-right font-semibold tabular-nums ${r.grossProfit >= 0 ? "text-emerald-700" : "text-red-600"}`}
                        >
                          {fmt(r.grossProfit)}
                        </td>
                      </tr>
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-border bg-muted/30 font-semibold">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums">
                        {fmt(totals.subscriptionRevenue)}
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums">
                        {fmt(totals.invoiceRevenue)}
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums">{fmt(totals.revenue)}</td>
                      <td className="px-3 py-2 text-right tabular-nums">
                        −{fmt(totals.expertPayouts)}
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums">
                        −{fmt(totals.agentCommissions)}
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums text-emerald-700">
                        {fmt(totals.platformFees)}
                      </td>
                      <td
                        className={`px-3 py-2 text-right tabular-nums ${totals.grossProfit >= 0 ? "text-emerald-700" : "text-red-600"}`}
                      >
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-border/60 bg-card">
                <div className="border-b border-border/60 px-4 py-3 flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {expenses?.payouts.length ?? 0} items
                  </span>
                </div>
                    ...
                    <div key={p.id} className="flex items-center justify-between px-4 py-2 text-sm">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {p.experts?.profession ?? p.kind} ·{" "}
                          {p.period_month?.slice(0, 7) ?? p.created_at?.slice(0, 10)}
                        </div>
                      </div>
              ...
              <div className="rounded-xl border border-border/60 bg-card">
                <div className="border-b border-border/60 px-4 py-3 flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {expenses?.commissions.length ?? 0} items
                  </span>
                </div>
                <div className="max-h-96 overflow-y-auto divide-y divide-border/40">
                  {(expenses?.commissions ?? []).slice(0, 20).map((c: any) => (
                    <div key={c.id} className="flex items-center justify-between px-4 py-2 text-sm">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge
                          variant={c.status === "paid" ? "outline" : "secondary"}
                          className="text-[10px]"
                        >
                          {c.status}
                        </Badge>
                        <span className="tabular-nums font-medium">
                          {fmt(Number(c.commission_eur ?? 0))}
                        </span>
                      </div>
                    </div>
                  ))}
