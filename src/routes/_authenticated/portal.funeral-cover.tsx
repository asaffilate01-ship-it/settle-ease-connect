  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      <header>
        ...
        <h1 className="display-lg mt-1 font-semibold">
          Family funeral cover — leads &amp; policies
        </h1>
        <p className="text-sm text-muted-foreground">
          Household leads from the upgrade page flow here for triage. Once an underwriter binds the
          policy, log it under "Policies" so the family sees it in-app.
        </p>
      </header>
            ...
            <ul className="space-y-3">
              {(leads.data as any[]).map((row) => (
                <LeadCard
                  ...
                  row={row}
                  onSet={(next, notes) =>
                    mut.mutate({ id: row.id, status: next, internal_notes: notes })
                  }
                />
              ))}
            </ul>
      ...
      {tab === "policies" && (
        <>
          {policies.isLoading ? (
            ...
            <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">
              No bound policies yet. When an underwriter confirms cover, log it here so the family
              sees it.
            </div>
          ) : (
            <ul className="space-y-3">
              {(policies.data as any[]).map((p) => (
                ...
                >
                  ...
                  <div className="mt-1 text-sm text-muted-foreground">
                    €{Number(p.benefit_eur).toLocaleString()} benefit · €
                    {Number(p.premium_eur).toFixed(2)} / {p.premium_cadence}
                    {" · "}
                    {p.adults_covered} adult{p.adults_covered === 1 ? "" : "s"}
                    {p.children_covered > 0 &&
                      ` + ${p.children_covered} child${p.children_covered === 1 ? "" : "ren"}`}
                  </div>
                </li>
              ))}
            </ul>
...
function LeadCard({
  ...
  row: any;
  onSet: (status: StatusKey, notes?: string) => void;
}) {
    ...
    <li className="rounded-2xl border border-border/60 bg-card p-4 shadow-soft">
      <div className="flex flex-wrap items-start gap-4">
        <div className="min-w-0 flex-1">
          ...
          {row.notes && (
            <p className="mt-2 whitespace-pre-line text-sm text-foreground/85">{row.notes}</p>
          )}
          <Textarea
            value={notes}
