...
import {
  ...
  listEmbassies,
  upsertEmbassy,
} from "@/lib/immigration.functions";
  ...
  head: () => ({
    meta: [
      { title: "Immigration ops — embassies & emergency alerts" },
      {
        name: "description",
        content:
          "Manage embassy directory and respond to emergency alerts raised by client contacts.",
      },
    ],
  }),
      ...
      <div className="flex flex-wrap gap-2 border-b border-border/60">
        {(["alerts", "embassies"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`rounded-t-lg px-4 py-2 text-sm font-medium capitalize ${tab === t ? "bg-card border border-b-transparent border-border/60" : "text-muted-foreground hover:text-foreground"}`}
          >
            {t}
          </button>
        ))}
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <AlertTriangle
                    className={`h-4 w-4 ${a.status === "open" ? "text-red-600" : "text-muted-foreground"}`}
                  />
                  <span className="font-semibold">{a.reason.replace(/_/g, " ")}</span>
                  <Badge variant="secondary">{a.status}</Badge>
                  <span className="text-xs text-muted-foreground">
                    Client: {a.profiles?.full_name ?? a.client_user_id.slice(0, 8)}
                  </span>
                </div>
                {a.description && (
              ...
              <div className="flex gap-2">
                {a.status === "open" && (
                  <Button size="sm" onClick={() => setStatus(a.id, "acknowledged")}>
                    Acknowledge
                  </Button>
                )}
                {a.status !== "resolved" && (
...
function EmbassiesTab() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    await upsert({
      data: {
        ...form,
        visa_services: (form.visa_services ?? "")
          .split(",")
          .map((s: string) => s.trim())
          .filter(Boolean),
    ...
    <section className="space-y-4">
      <div className="flex justify-end">
        <Button
          onClick={() =>
            setForm(
              form ? null : { mission_type: "embassy", visa_services: "", languages: "en,de" },
            )
          }
            ...
            <div className="mt-2 flex justify-end">
              <Button
                ...
                onClick={() =>
                  setForm({
                    ...e,
                    visa_services: (e.visa_services ?? []).join(", "),
