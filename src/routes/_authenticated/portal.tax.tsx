...
function TaxConsole() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {tab === "leads" && (
        <div className="space-y-6">
          <div className="grid gap-3 sm:grid-cols-4">
            ...
            <Kpi
              label="Est. refund pool"
              value={`€${data.estRefundTotal.toLocaleString()}`}
              tone="primary"
            />
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Tax year {l.tax_year ?? "—"} · Income €
                      {Number(l.gross_income_eur ?? 0).toLocaleString()} · {l.source ?? "web"}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-sm tabular-nums text-emerald-700">
                      {l.estimated_refund_eur
                        ? `€${Number(l.estimated_refund_eur).toLocaleString()}`
                        : "—"}
                    </span>
                    <Badge
                      variant={l.status === "filed" || l.status === "won" ? "default" : "secondary"}
                      className="text-[10px] uppercase"
                    >
                      {l.status === "filed" || l.status === "won" ? (
      ...
      {tab === "quotes" && (
        <EmptyTab>
          Tax filings are quoted via the case workspace. Open a lead to send a quote.
        </EmptyTab>
      )}
      {tab === "callbacks" && (
        <EmptyTab>
          No callback queue for tax leads — advisors reach out via message on the case.
        </EmptyTab>
      )}
      {tab === "reconciliation" && (
