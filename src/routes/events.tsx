...
export const Route = createFileRoute("/events")({
  head: () => ({
    meta: [
      { title: "Community events — BeistandPlus" },
      {
        name: "description",
        content:
          "Browse events that have been published in the BeistandPlus community calendar and review each event's stated organiser, fee and terms.",
      },
      { property: "og:title", content: "Community events — BeistandPlus" },
      {
        property: "og:description",
        content: "Browse published community events and review the details before registering.",
      },
...
function CategoryIcon({ category, sub }: { category: string; sub: string | null }) {
  const Icon =
    (sub && TOPIC_ICON[sub]) ||
    (category === "trip" ? Bus : category === "advice_clinic" ? Stethoscope : Users);
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl space-y-10 px-4 py-12">
        <header className="space-y-3">
          <h1 className="display-lg font-semibold">Community events</h1>
          <p className="max-w-2xl text-muted-foreground">
            Browse events published in the BeistandPlus calendar. Check the named organiser, venue,
            price, accessibility and any professional status directly before relying on the listing
            or registering.
          </p>
        </header>
