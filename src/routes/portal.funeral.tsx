...
export const Route = createFileRoute("/portal/funeral")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Receive referrals, upload quotes and invoices, and coordinate with families in one portal." },
    ],
  }),
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 p-6 lg:p-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Portal</div>
            <h1 className="mt-1 font-display text-3xl font-semibold">Furkan Bestattungen · Berlin</h1>
          </div>
              ...
              {referrals.map((c) => (
                <tr key={c.id} className="hover:bg-parchment/40">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-muted-foreground">{c.id} · {c.city}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div>{c.religion}</div>
                    <div className="text-xs text-muted-foreground capitalize">{c.disposition}{c.destination ? ` → ${c.destination}` : ""}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link to="/app/cases/$caseId" params={{ caseId: c.id }} className="text-sm text-primary hover:underline">
                      Open →
                    </Link>
                  </td>
                </tr>
