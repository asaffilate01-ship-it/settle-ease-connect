...
export const recordDelaConsent = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAal2])
  .validator((d: { referralId: string; evidence: string }) => z.object({
...
export const sendDelaToPartner = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAal2])
  .validator((d: { referralId: string; partnerId?: string }) => z.object({
    referralId: z.string().uuid(),
...
export const setHealthTriage = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAal2])
  .validator((d: { insuranceLeadId: string; route: (typeof HEALTH_TRIAGE_ROUTES)[number]; notes?: string }) =>
    z.object({
