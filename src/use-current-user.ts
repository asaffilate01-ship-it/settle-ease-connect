...
const nav: NavItem[] = [
  ...
  { to: "/portal/escrow", labelKey: "sidebar.escrow", icon: "benefits", groupKey: "sidebar.internal", requiresRole: "internal", audience: "internal", specialistDomain: "finance" },
  { to: "/portal/financials", labelKey: "sidebar.financials", icon: "benefits", groupKey: "sidebar.internal", requiresRole: "internal", audience: "internal", specialistDomain: "finance" },
...
const ROLE_LABEL: Record<AppRole, string> = {
  ...
  family_deputy: "Family Deputy",
  senior_case_manager: "Senior Case Manager",
...
const ROLE_TONE: Record<AppRole, string> = {
  ...
  family_deputy: "bg-primary/15 text-primary",
  senior_case_manager: "bg-violet-500/15 text-violet-600 dark:text-violet-300",
  team_leader: "bg-purple-500/15 text-purple-600 dark:text-purple-300",
  finance: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300",
  compliance: "bg-slate-500/15 text-slate-700 dark:text-slate-300",
  dpo: "bg-slate-500/15 text-slate-700 dark:text-slate-300",
  auditor: "bg-slate-500/15 text-slate-700 dark:text-slate-300",
};
...
export function AppSidebar() {
  ...
  const isBroadInternal =
    isAdmin
    || roles.includes("staff")
    || roles.includes("case_manager")
    || roles.includes("senior_case_manager")
  ...
  const isFinanceOnly = roles.includes("finance") && !isBroadInternal;
  ...
  const visibleNav = nav.filter((n) => {
    ...
    if (isSpecialist && !isBroadInternal) {
      if (isFinanceOnly && n.to !== "/portal" && n.specialistDomain !== "finance") return false;
  ...
  return (
    <Sidebar collapsible="offcanvas" className="border-e border-sidebar-border bg-sidebar text-sidebar-foreground">
      <Link
        to="/app"
